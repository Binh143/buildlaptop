$(document).ready(function () {
  ...
  $(".carousel").on("click", ".btn-control, .preview", function () {
    ...
    if (current < marker) {
      ...
      $(this)
        .parents(".carousel")
        .find('.items .item[data-marker="' + current + '"]')
        .removeClass("center")
        .removeClass("center-left")
        .removeClass("center-right")
        .removeClass("right-center")
        .removeClass("left-center")
        .addClass("center-left");
      $(this)
        .parents(".carousel")
        .find('.items .item[data-marker="' + marker + '"]')
        .removeClass("center")
        .removeClass("center-left")
        .removeClass("center-right")
        .removeClass("right-center")
    ...
    $(this)
      .parents(".carousel")
      .find('.markers > li[data-marker="' + marker + '"]')
      .addClass("active");
  });
        ...
        if (phase == "start" || phase == "move") {
          ...
          if (phase == "move") {
            if (direction == "left" && distance >= 100) {
              $(this)
                .find('.btn-control[data-direction="right"]')
                .trigger("click");
              return false;
            } else if (direction == "right" && distance >= 100) {
              $(this)
                .find('.btn-control[data-direction="left"]')
                .trigger("click");
              return false;
            }
          }
  ...
  $("body")
    ...
    .each(function (index) {
      if (parseInt($(this).attr("data-count")) >= 3) {
        ...
        var nextimagesrc = $(this)
          .find('.item[data-marker="' + next + '"] img')
          .attr("src");
        var previmagesrc = $(this)
          .find('.item[data-marker="' + prev + '"] img')
          .attr("src");

        $(this)
          .find('.preview[data-direction="right"]')
          .css({
            background:
              "url(" + nextimagesrc + ") no-repeat center center scroll",
        ...
        $(this)
          .find('.preview[data-direction="left"]')
          .css({
            background:
              "url(" + previmagesrc + ") no-repeat center center scroll",
            "-webkit-background-size": "cover",
            "-moz-background-size": "cover",
            "-o-background-size": "cover",
            "background-size": "cover",
